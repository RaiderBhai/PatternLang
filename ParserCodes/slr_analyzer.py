"""
SLR(1) (Simple LR) Parser Analyzer
Uses LR(0) items with FOLLOW sets for lookahead
"""
...
class LR0State:
    """Represents a state in LR(0) automaton (used by SLR)"""
...
class SLRAnalyzer:
    ...
    def compute_follow(self):
                ...
                for production in self.grammar.get(A, []):
                    for i, symbol in enumerate(production):
                        if symbol in self.non_terminals:
                            # For each B in production A → αBβ
                            beta = production[i+1:]
                            ...
                            if beta:
                                ...
                                for b_symbol in beta:
                                    ...
                                    if len(self.follow[symbol]) > before:
                                        changed = True
                                    # If b_symbol is terminal or non-nullable non-terminal, stop
                                    ...
                                    before = len(self.follow[symbol])
                                    self.follow[symbol].update(self.follow.get(A, set()))
                                    if len(self.follow[symbol]) > before:
                                        changed = True
                            else:
                                # β is empty, so add FOLLOW(A) to FOLLOW(B)
                                before = len(self.follow[symbol])
                                self.follow[symbol].update(self.follow.get(A, set()))
                                if len(self.follow[symbol]) > before:
                                    changed = True
        ...
        for A in sorted(self.non_terminals):
            if A != self.augmented_start:  # Don't show augmented start
    ...
    def build_automaton(self):
        """Build LR(0) automaton (same as LR(0), but will use FOLLOW for parsing)"""
        print("\n=== BUILDING LR(0) AUTOMATON ===\n")
        print("Note: SLR uses LR(0) items with FOLLOW sets")
    ...
    def build_parsing_tables(self):
        ...
        for state in self.states:
            ...
            # Populate action table and detect conflicts
    ...
    def generate_excel(self, filename='slr_analysis.xlsx'):
    ...
    def create_result_sheet(self, wb):
        ...
        is_slr = self.is_slr()
        ws['B3'] = "YES ✓" if is_slr else "NO ✗"
        ws['B3'].font = Font(bold=True, size=14, color="008000" if is_slr else "FF0000")
    ...
    def create_first_sheet(self, wb):
        ...
        for A in sorted(self.non_terminals):
            ...
            ws[f'C{row}'] = "Used for computing FOLLOW sets"
            row += 1
    ...
    def create_follow_sheet(self, wb):
        ...
        for A in sorted(self.non_terminals):
            if A != self.augmented_start:  # Don't show augmented start
                ...
                ws[f'C{row}'] = "Used for SLR reduce actions"
                row += 1
...
def main():
    ...
    print("="*70)
    print("SLR(1) (SIMPLE LR) PARSER ANALYZER")
    print("="*70)
    
    analyzer = SLRAnalyzer(grammar)
    ...
    if analyzer.is_slr():
        ...
        print("\nSLR uses LR(0) items with FOLLOW sets for lookahead.")
        print("More powerful than LR(0), less powerful than CLR (LR(1)).")
    else:
        ...
        print("\n💡 Suggestion: Try CLR (LR(1)) parser for this grammar.")
